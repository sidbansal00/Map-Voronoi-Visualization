...
def find_closest(location, centroids):
    ...
    [2.0, 3.0]
    """
    return min(centroids, key = lambda individual_centroid: distance(location, individual_centroid))
...
def find_centroid(cluster):
    ...
    first, second = [],[]
    for x in cluster:
        first.append(restaurant_location(x)[0])
        second.append(restaurant_location(x)[1])
...
def find_predictor(user, restaurants, feature_fn):
    ...
    meanxs, meanys = mean(xs), mean(ys)
    totxs = sum([pow(x-meanxs,2) for x in xs])
    totys = sum([pow(y-meanys,2) for y in ys])
    l = [(x-meanxs) for x in xs]
    m = [(y-meanys) for y in ys]
    totxy = sum([n[0]*n[1] for n in zip(l,m)])
    b = totxy/totxs
    a = meanys - (b*meanxs)
    r_squared = pow(totxy,2)/(totxs* totys)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for x in feature_fns:
        if test < find_predictor(user, reviewed,x)[1]:
            test = find_predictor(user, reviewed,x)[1]
            predictor = find_predictor(user, reviewed,x)[0]
